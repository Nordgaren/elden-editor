...
pub struct THROW_PARAM_ST {
	...
	pub sphereCastRadiusRateTop:u16,
	...
	pub sphereCastRadiusRateLow:u16,
...
impl THROW_PARAM_ST {
	...
	/// Bitfield1
	pub fn get_isTurnAtker(&self) -> bool {
		&self.Bitfield1 & 0x1 != 0
	}

	/// Bitfield1
	pub fn set_isTurnAtker(&mut self, state: bool) {
		if state {
			self.Bitfield1 |= 0x1
		} else {
			self.Bitfield1 &= 0xFE
	...
	/// Do you want to skip the attacker's weapon category check? - 攻撃側の武器カテゴリチェックをスキップするか？
	/// Bitfield1
	pub fn get_isSkipWepCate(&self) -> bool {
		&self.Bitfield1 & 0x2 != 0
	}

	/// Bitfield1
	pub fn set_isSkipWepCate(&mut self, state: bool) {
		if state {
			self.Bitfield1 |= 0x2
		} else {
			self.Bitfield1 &= 0xFD
	...
	/// Do you want to skip the sphere cast? - スフィアキャストをスキップするか？
	/// Bitfield1
	pub fn get_isSkipSphereCast(&self) -> bool {
		&self.Bitfield1 & 0x4 != 0
	}

	/// Bitfield1
	pub fn set_isSkipSphereCast(&mut self, state: bool) {
		if state {
			self.Bitfield1 |= 0x4
		} else {
			self.Bitfield1 &= 0xFB
	...
	/// If it is set to ◯, the location of the throwing position will be the "position of the adsorption Damipoli that has returned to the positional relationship equivalent to the flat ground". - ◯にすると投げ位置合わせの場所が「平地相当の位置関係に戻した吸着ダミポリの位置」になる
	/// Bitfield1
	pub fn get_isEnableCorrectPos_forThrowAdjust(&self) -> bool {
		&self.Bitfield1 & 0x8 != 0
	}

	/// Bitfield1
	pub fn set_isEnableCorrectPos_forThrowAdjust(&mut self, state: bool) {
		if state {
			self.Bitfield1 |= 0x8
		} else {
			self.Bitfield1 &= 0xF7
	...
	/// Is the same treatment as the wall removal prevention applied to the fall prevention wall so that it does not fall from the step when the follow-up is released? - 追従解除時に段差から落下しないよう、壁抜け防止と同じ処理を落下防止壁に対しても行うか？
	/// Bitfield1
	pub fn get_isEnableThrowFollowingFallAssist(&self) -> bool {
		&self.Bitfield1 & 0x10 != 0
	}

	/// Bitfield1
	pub fn set_isEnableThrowFollowingFallAssist(&mut self, state: bool) {
		if state {
			self.Bitfield1 |= 0x10
		} else {
			self.Bitfield1 &= 0xEF
	...
	/// Do you want to return the character with the Adsorbed Damipoli when it comes in contact with a hit or fall prevention wall so that it does not look like it will get stuck in the middle wall or fall from a step? - 追従中壁にめり込んだり段差から落下しそうな見た目にならないよう、ヒットや落下防止壁に接触したときに吸着ダミポリ所持キャラごと戻す処理を行うか？
	/// Bitfield1
	pub fn get_isEnableThrowFollowingFeedback(&self) -> bool {
		&self.Bitfield1 & 0x20 != 0
	}

	/// Bitfield1
	pub fn set_isEnableThrowFollowingFeedback(&mut self, state: bool) {
		if state {
			self.Bitfield1 |= 0x20
		} else {
			self.Bitfield1 &= 0xDF
	...
	/// pad - pad
	/// Bitfield1
	pub fn get_pad0(&self) -> u8 {
		&self.Bitfield1 & 0xC0
	}

	/// Bitfield1 MAX: 3
	pub fn set_pad0(&mut self, state: u8) {
		if state != 0 {
